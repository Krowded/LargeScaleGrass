...
//	theMesh->vertexToTriangleTable = NULL;
	...
	if (hasPositionIndices)
		theMesh->coordIndex = (GLint*)malloc(sizeof(int) * coordCount);
	if (hasNormalIndices)
		theMesh->normalsIndex = (GLint*)malloc(sizeof(int) * coordCount);
	if (hasTexCoordIndices)
		theMesh->textureIndex = (GLint*)malloc(sizeof(int) * coordCount);
